...
open class GitUpdateTask constructor(
    ...
    fun checkLocalGitRepo() {
        if (File(workspace, ".git").exists()) {
            val localUrl = CommonShellUtils.execute(
            ...
            val localUrlObj = kotlin.runCatching { getUrlHostAndPath(localUrl) }
                .onFailure {
                    logger.warn("获取本地git代码库主机信息失败: $localUrl")
                }.getOrNull()
            val url = getUrlHostAndPath(atomParam.repositoryUrl)
            // 获取本地数据失败，说明前次操作有脏数据
            if (localUrlObj == null || localUrlObj.first != url.first && localUrlObj.second != url.second) {
    ...
    private fun setConfig() {
        ...
        CommonShellUtils.execute("git config user.email ${atomParam.pipelineStartUserName}@devops.com", workspace)
